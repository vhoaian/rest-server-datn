...
const {nomalizeResponse} = require("../utils/normalize");
...
const loginWithGoogleAccount = async (req, res) =>{
    ...
    const {success, message, data} = await AuthService.loginWithGoogle(id, accessToken);
...
const vertifyPhoneNumber = async (req,res,next)=>{
    ...
    const {success, message, data} = await AuthService.registerNewUser(null, userID, phone);
    console.log( {success, message, data} );
    res.send(nomalizeResponse(success, message, data));
}
